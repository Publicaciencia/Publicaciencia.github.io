const PUBLICA_CONFIG={appsScriptWebAppUrl:"TU_URL_WEB_APP_APPS_SCRIPT"};
...
function setupUploadForm(){
  ...
  form.addEventListener("submit",e=>{
    if(PUBLICA_CONFIG.appsScriptWebAppUrl.includes("TU_URL")){
      e.preventDefault(); alert("Falta configurar la URL del receptor."); return;
    }
    const f=document.getElementById("pdfFile").files[0];
    if(!f){e.preventDefault();return}
    if(f.size>15*1024*1024){e.preventDefault();alert("El PDF supera 15 MB.");return}
    e.preventDefault();
    const btn=document.getElementById("sendBtn");
    btn.disabled=true; btn.textContent="Preparando envío...";
...
document.addEventListener("DOMContentLoaded",()=>{filterWorks();setupUploadForm()});
